 ...
 */
public int[][] createMatrix (int m,int n)throws Exception{
    if (n <= 0 || m <= 0){
    ...
    for (int i = 0; i < m; i++){
        for (int j = 0; j < n; j++){
            ...
            matrix[i][j] = (sign > 0.5) ? (int)(Math.random()*10) : -(int)(Math.random()*10);
        }
    }
    return matrix;
}
     ...
     *@Author Sherlock.Xiao
     */
    public int[][] strassenSquareMultiply(int[][] A, int[][] B) throws Exception {
...
//        }
        ...
        int[][] A11 = divideSquareMatrix(A,1);
        int[][] A12 = divideSquareMatrix(A,2);
        int[][] A21 = divideSquareMatrix(A,3);
        int[][] A22 = divideSquareMatrix(A,4);
        int[][] B11 = divideSquareMatrix(B,1);
        int[][] B12 = divideSquareMatrix(B,2);
        int[][] B21 = divideSquareMatrix(B,3);
        int[][] B22 = divideSquareMatrix(B,4);
        ...
        int[][] C11 = add(minus(add(P[3],P[4]),P[1]),P[5]);
        int[][] C12 = add(P[0],P[1]);
        int[][] C21 = add(P[2],P[3]);
        int[][] C22 = minus(minus(add(P[4],P[0]),P[2]),P[6]);
        C = mergeFourMatrix(C11,C12,C21,C22);

        return C;
    }

    public int[][] add(int[][] M, int[][] N)throws Exception{
    ...
    public int[][] minus(int[][] M, int[][] N)throws Exception{
        ...
        int[][] result = new int[rowOfM][colOfM];
     ...
     *@Author Sherlock.Xiao
     */
    public int[][] divideSquareMatrix(int[][] A, int k)throws Exception{
     ...
     *@Author Sherlock.Xiao
     */
    public int[][] mergeFourMatrix(int[][] A11, int[][] A12, int[][] A21, int[][] A22)throws Exception{
        int m = A11.length;
        int n = A11[0].length;
