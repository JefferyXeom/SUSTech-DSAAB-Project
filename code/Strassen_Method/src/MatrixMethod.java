...
public class MatrixMethod {
    ...
    private static boolean stranssenThreshold = false;
    ...
    public static void setStranssenThreshold(boolean t) {
        stranssenThreshold = t;
    }
    ...
    private static int extensionCheck(int n) {
        int num = 1;
        int[] splits = getK(n);
        int k = splits[0];
        int m = splits[1];
    ...
    private static boolean isStandard(int num) {
    ...
    public static Matrix stranssenMultiple(Matrix A, Matrix B) throws Exception {
        ...
        if (isStandard(n)) {
            ...
            return new Matrix((stranssenMultiples(A.getMatrix(), B.getMatrix(), 0)));
        } else {
            int num = extensionCheck(n);
            double[] AA = matrixExtension(A.getMatrix(), num);
            double[] BB = matrixExtension(B.getMatrix(), num);
//            System.out.println(Arrays.toString(AA));
//            System.out.println(Arrays.toString(BB));
            return new Matrix(stranssenMultiples(AA, BB, num));
        }

    }

    private static double[] matrixExtension(double[] A, int num) {
        int n = A.length;
        int m = (int) Math.sqrt(n);
        double[] AA = new double[(m + num) * (m + num)];
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < m; j++) {
                AA[i * (m + num) + j] = A[i * m + j];
            }
            for (int j = 0; j < num; j++) {
                AA[i * (m + num) + m + 1 + j] = 0;
            }
    ...
    private static double[] matrixShrink(double[] A, int num) {
        int n = A.length;
        int m = (int) Math.sqrt(n);
        double[] AA = new double[(m - num) * (m - num)];
    ...
    private static int[] getFactor(int num) {
        ...
        int test = 0;
        int approximate_sqrt = (int) Math.sqrt(num);
    ...
    public static double[] naiveSubMultiple(double[] A, double[] B) {
        int[] factor = getFactor((int) Math.sqrt(A.length));// A and B have the same length
        int k = factor[0];
        int m = factor[1];
//        System.out.println(A.length + " " + k + " " + m);
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < m; j++) {
                ...
                for (int p = 0; p < m; p++) {
                    ...
                    MC = naiveAdd(MC, naiveMultiple(MA, MB));
                }
                NC[i * m + j] = MC;
            }
        }
    ...
    private static double[] stranssenMultiples(double[] A, double[] B, int num) throws Exception {
        int[] splits = getK(A.length);// A and B have the same length
        int k = (int) Math.pow(2, splits[0] / 2);//2
...
//        System.out.println(A.length + " " + k + " " + m);
    ...
    private static double[] standardStranssenMultiples(double[] A, double[] B) throws Exception {
        ...
        if (!stranssenThreshold || n > threshold) {
...
//      int row = m / 2,
    ...
    public static double[] mergeFourMatrix(double[] A11, double[] A12, double[] A21, double[] A22) throws
            Exception {
